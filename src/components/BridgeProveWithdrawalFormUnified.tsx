...
const BridgeProveWithdrawalFormUnified = (props: PropsFromRedux & IBridgeProveWithdrawalForm) => {
  ...
  const { 
    executeTransaction: executeProveWithdrawalTx,
    isAwaitingWalletInteraction: isAwaitingWalletInteractionProveWithdrawalTx,
    isAwaitingTx: isAwaitingProveWithdrawalTx,
    isLoading: isLoadingProveWithdrawalTx,
    ...
    successToastMessage: `Proof submission success! Please note that after ~ 1 week you will need to come back and finalize your withdrawal.`,
    fallbackErrorMessage: "Unable to submit proof, please refresh the page and try again, or contact support if the problem persists.",
  });
    ...
    void (async () => {
      try {
        if(proveWithdrawalConfig) {
          await executeProveWithdrawalTx(proveWithdrawalConfig);
        } else {
        ...
        {transactionData && !isLoading &&
          <div className={classes.cardInner}>
            ...
            <FloatingActionButton
              ...
              disabled={!isInitialized || isLoadingProveWithdrawalTx || isAwaitingWalletInteractionProveWithdrawalTx || isAwaitingProveWithdrawalTx || isAwaitingValidPreparation || isWithdrawalAlreadyProven || showSuccessMessage}
              onClick={handleProveWithdrawal}
              showLoadingIcon={!isInitialized || isAwaitingWalletInteractionProveWithdrawalTx || isAwaitingProveWithdrawalTx || isAwaitingValidPreparation}
              text={getBridgeProveWithdrawalButtonText(isInitialized, isAwaitingWalletInteractionProveWithdrawalTx, isAwaitingProveWithdrawalTx, isAwaitingValidPreparation, isWithdrawalAlreadyProven, showSuccessMessage)}
            />
...
export default BridgeProveWithdrawalFormUnified;